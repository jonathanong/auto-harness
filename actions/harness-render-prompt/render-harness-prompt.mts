...
You are running in an isolated CI-only checkout and Auto Harness worktree. Treat the current directory as the workspace and read the repository instructions before acting. Repository dependencies are not preinstalled; initialize only the tooling your task needs with the repository's pinned package manager. Do not create a second worktree. Do not run any repository-specific worktree-reset script; either action can discard or detach the session state. This session is non-interactive: it ends when you send your final response, and any background command still running ends with it, so its result is never acted on. Run waits, polls, and retry loops in the foreground; when a tool time limit cuts one short, re-run it in the foreground. Never send your final response while a background command you started is still running. Follow the repository's ordinary implementation, validation, commit, push, and pull-request rules whenever the task authorizes those actions. Never expose credentials, raw environment values, or other secrets in logs, artifacts, commits, comments, or pull requests.`;
