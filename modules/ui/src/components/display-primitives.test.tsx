...
describe("shared display primitives", () => {
  it("merges utility classes and exposes every badge and button variant", () => {
  ...
  it("keeps semantic structure, labels, and input attributes intact", () => {
    const card = render(
  ...
  it("renders accessible table structure and status meanings", () => {
    const table = render(
      <Table className="table-marker" aria-label="Sessions">
        <TableHeader className="header-row-marker">
          <TableRow className="row-marker">
            <TableHead scope="col" className="head-marker">
              Status
            </TableHead>
          </TableRow>
        </TableHeader>
    ...
    expect(table).toContain("header-row-marker");
  ...
  it("preserves text and link semantics when tooltips provide additional guidance", () => {
    const text = render(
