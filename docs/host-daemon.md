...
| `prompt`          | Already appended as the final `resolvedArgv` element when the Command's `appendPrompt` is true — **never** interpolated into a shell string. A literal `--` is inserted first only when the Command opts in via `appendPromptSeparator` (safe only for getopt-style executables; e.g. `printf "%s"` reads `--` as data, not a terminator, so it defaults off). |
