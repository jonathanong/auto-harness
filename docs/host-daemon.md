...
8. On exit / timeout / cancel, run the terminal hook, collect the structured result, then send `session:status`, release claim/lock, and emit worktree status. For a v4 first-attempt `checkout_fetch_failed`, the daemon instead retains the hook until the durable `session:status-acknowledged` disposition arrives: it discards it only when `retryAccepted: true`, and runs it when the disposition is terminal. The retry runs its own hook if it fails and exhausts the budget. Once a retry is durably queued, a later `queue_expired` is the normal control-plane-only queued terminal path; queued sessions have no daemon hook owner. For a non-replay-safe or exhausted `host_lost` outcome, a v5 control plane persists a bounded hook handoff to the original host. Its replacement daemon claims the original target, resolves its current local hook policy, runs (or fail-closed no-ops) the hook, and durably acknowledges it before the terminal session is archived. If that host never returns, the handoff expires after 24 hours and records the no-op rather than retaining an active-host index indefinitely.
