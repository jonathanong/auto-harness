...
workflow. Normal pull requests must leave `modules/client/package.json` at its current version; CI
rejects version changes in pull requests.
...
3. Wait for the workflow to validate the package, commit the selected version to `main`, create the
   matching `client-v<version>` tag, and publish with npm provenance.
4. Confirm the workflow succeeded and the exact version is visible on npm.
...
- If a rerun reports that it was already published or superseded, inspect the original run, the
  `client-v<version>` tag, and npm. Do not delete or move a release tag. Start a fresh workflow
  dispatch only when another version is intended.
