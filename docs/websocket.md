...
| `session:command-start-acknowledged` | `sessionId`, `attemptId`                                                                                                                                                                                                                                                                                                   | Durable authorization to spawn the primary CLI. The control plane fences this to the current assignment attempt.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| `session:status-acknowledged`        | `sessionId`, `attemptId`, optional `retryAccepted`, optional `terminalHookHandoffId`, optional `terminalHookHandoffExpiresAt`                                                                                                                                                                                              | Durable application of a terminal report. For a first-attempt `checkout_fetch_failed`, `retryAccepted: true` transfers terminal-hook ownership to the accepted retry. A terminal disposition — including storage-less exhausted `checkout_fetch_failed` — carries the durable handoff id; the reporting daemon runs its retained hook, recollects the structured result, and completes that handoff before archival. If operator cancellation or a running-timeout sweep already committed, a deferred checkout-failure still receives that handoff id (and expiry) on durable and storage-less paths so the daemon can complete the retained hook exactly once.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| `host:registered`                    | `hostId`, `connectionId?`, `protocolVersion?`                                                                                                                                                                                                                                                                              | Durable acknowledgement that `host:register` was accepted. `protocolVersion` is the control plane's protocol (`7`).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| `host:keepalive-ack`                 | `hostId`, `at`                                                                                                                                                                                                                                                                                                             | Durable acknowledgement that `host:keepalive` committed (`at` echoed). Sent via `postToConnection` / the local hub socket — never as a Lambda `$default` response body. The daemon re-arms the stall watchdog only on this frame (or `host:registered`), not on a local `send()`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
| Type                             | Payload                                                                                                                                                                                                                                                                                                                                   | Purpose                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| -------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `host:register`                  | `hostId`, `worktrees[]`, `protocolVersion`, `runtime`, `daemonInstanceId`, `daemonStartedAt`, `runningAttempts[]`, optional `repositories[]`, optional `workspacePools[]`, optional `capabilities` (`{ features[], maxConcurrentAssignments }`), optional `providerAccountReadiness[]`, optional `runningSessions[]`, optional `draining` | Inventory, repository/workspace-slot metadata, capabilities including assignment capacity, opaque per-account execution-profile readiness, Git checkout-recovery readiness, reclaim after reconnect (attempt-fenced), and a reconnecting drain intent. `protocolVersion` must be `7`. `runtime.gitReady` is required. `daemonInstanceId` and `daemonStartedAt` are a required pair. Omitted `runningSessions` is treated as an empty list and reconciled.                                                            |
...
| `host:drain`                         | _(none)_                                                                                                         | Operator-initiated drain, pushed by `POST /api/v1/hosts/drain`. Distinct from the `host:draining`/`host:status` pair above: this is the control plane telling the daemon to drain, not acknowledging a daemon-initiated request. The daemon stops accepting new `session:assign` immediately (no ack required) but keeps in-flight sessions running.                                                                                                                                                                                                                                                                                                                                                                                                         |
| `host:resume`                        | _(none)_                                                                                                         | Inverse of `host:drain`, pushed by `POST /api/v1/hosts/resume`. The daemon calls its internal `resumeFromDrain()` and re-registers, restoring capacity. Added without a `HOST_PROTOCOL_VERSION` bump — see the note below.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
**Draining (operator):** `POST /api/v1/hosts/drain` and `POST /api/v1/hosts/resume` (see [api.md](api.md)) push the bare `host:drain`/`host:resume` messages above instead. There is no daemon-initiated ack for either — the control plane commits the durable `draining` flag (fenced to the current lease owner) before publishing, so a slow or dropped WebSocket write cannot desync the two. `HOST_PROTOCOL_VERSION` gates only the `host:register` handshake, not individual message types, and an older daemon's handler silently ignores an unrecognized type — so `host:resume` needed no protocol bump, at the cost of a daemon on stale code not resuming until it next restarts or auto-updates.
...
forwards that buffered `session:status` result on completion. The current protocol is
`HOST_PROTOCOL_VERSION` `7`. Registration requires that exact integer; anything else is rejected
as an unsupported host protocol. The durable pre-launch `session:command-start` handshake records
...
the one bounded infrastructure retry; post-launch or ambiguous loss remains terminal. Terminal
`session:status` always carries the bounded `SessionResult`. The daemon re-arms its stall watchdog
on `host:keepalive-ack` / `host:registered` rather than on a local keepalive write.
`session:log` requires `attemptId`. The control plane ignores delayed ACK, cancel,
reconnect-claim, status, usage, and log frames whose `attemptId` is no longer current,
