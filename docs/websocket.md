...
| `host:registered`    | `hostId`, `connectionId?`, `protocolVersion?`                                                                                                                                                                                                               | Durable acknowledgement that `host:register` was accepted. `protocolVersion` is the control plane's protocol (currently `2`); omit it and a v2 daemon keeps send-based keepalive re-arm for mixed rollout                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
Version `3` adds the bounded `SessionResult` on terminal `session:status`; the daemon sends it only
when the server advertises version 3 or newer, and the server accepts it only on a version-3 host
connection. Version `2` is keepalive-ack: the daemon re-arms its stall watchdog on `host:keepalive-ack` /
