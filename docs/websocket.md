...
| `session:status-acknowledged`        | `sessionId`, `attemptId`, optional `retryAccepted`                                                                                                                                                                                                                                       | Durable application of a terminal report. For a v4 first-attempt `checkout_fetch_failed`, `true` transfers terminal-hook ownership to the accepted retry; `false` makes the reporting daemon run its retained hook.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| `host:registered`                    | `hostId`, `connectionId?`, `protocolVersion?`                                                                                                                                                                                                                                            | Durable acknowledgement that `host:register` was accepted. `protocolVersion` is the control plane's protocol (currently `5`); older daemons keep their negotiated behavior during rollout                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
| Type                             | Payload                                                                                                                                                                                                                                                                                                                  | Purpose                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| -------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `host:register`                  | `hostId`, `worktrees[]`, optional `repositories[]`, optional `capabilities` (legacy feature array or `{ features[], maxConcurrentAssignments }`), optional `providerAccountReadiness[]`, optional `runtime`, optional `runningSessions[]`, optional `runningAttempts[]`, optional `protocolVersion`, optional `draining` | Inventory, repository metadata, rollout capabilities including assignment capacity, opaque per-account execution-profile readiness, Git checkout-recovery readiness, reclaim after reconnect (attempt-fenced), protocol negotiation, and a reconnecting drain intent                                                                                                                                                                               |
...
| `session:status`                 | `sessionId`, `worktreeId`, `attemptId`, `status`, `exitCode?`, `errorCode?`, `errorMessage?`, `result?`                                                                                                                                                                                                                  | Lifecycle (`running`, `completed`, `failed`, `cancelled`, `timed_out`); echoes the assignment fence. Terminal status persists a captured native resume ref and best-effort structured result when available. On AI quota hits: `failed` + `errorCode: "usage_limit"`; infrastructure failures use `checkout_fetch_failed` or `host_lost` under the bounded retry policy (see [host-daemon.md](host-daemon.md#usage-limits-ai-vendor--cli-quotas)). |
...
| `session:terminal-hook-complete` | `sessionId`, `handoffId`                                                                                                                                                                                                                                                                                                 | v5 replacement-daemon confirmation after it ran or safely no-oped a retained host-loss terminal hook.                                                                                                                                                                                                                                                                                                                                              |
...
| `session:terminal-hook`              | `sessionId`, `handoffId`, `repositoryId`, `worktreeId`, `status`, `errorCode?`, `ref?`, `metadata?` | v5 only: a crashed daemon's host-loss hook. The replacement daemon resolves its live local policy, never a server-provided script path.        |
...
Modern daemons advertise `protocolVersion` (currently `5`) and `runningAttempts: [{ sessionId, attemptId }]`.
