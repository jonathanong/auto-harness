...
Modern daemons advertise `protocolVersion` (currently `3`) and `runningAttempts: [{ sessionId, attemptId }]`.
Version `3` adds the durable pre-launch `session:command-start` handshake: the control plane records
...
the one bounded infrastructure retry; post-launch or ambiguous loss remains terminal. It also adds
the bounded `SessionResult` on terminal `session:status`; the daemon sends it only when the server
advertises version 3 or newer, and the server accepts it only on a version-3 host connection.
