...
| `session:status`  | `sessionId`, `worktreeId`, `workspaceSlotId?`, `workspaceSlotError?`, `attemptId`, `status`, `exitCode?`, `errorCode?`, `errorMessage?`, `result?`                                                                                                                                                                                                    | Lifecycle (`running`, `completed`, `failed`, `cancelled`, `timed_out`); echoes the assignment fence. Workspace cleanup diagnostics use `workspaceSlotError` and terminal `errorCode: "workspace_cleanup_failed"`. Terminal status may also include a bounded structured result.     |
