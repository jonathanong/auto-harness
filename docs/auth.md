...
The viewer WebSocket also requires a browser `Origin` that matches the configured web origin (`PUBLIC_BASE_URL` / ControlPlane `publicBaseUrl`). A missing or mismatched `Origin` is rejected.
