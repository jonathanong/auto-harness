...
Session path after the host daemon is online: `POST /api/v1/sessions` (optionally with a global exact `concurrencyId`). The local API starts a scheduler sweep on startup and repeats it every minute; `POST /api/v1/scheduler/assign` remains available to force a sweep. A duplicate active identity returns `200`/`created: false`; a new session returns `201`/`created: true`. Details: [host-daemon-e2e-testing.md](host-daemon-e2e-testing.md).
