...
Create a mode-`0600` config file outside the checkout, then set its native-absolute path as
...
daemon-user-writable PATH directory. `sslCAInfo` must be an absolute regular file whose path and
every ancestor are root-owned, non-writable, and symlink-free. `httpProxy` must
...
(including when the execution profile is unavailable after minting, or when `resolvedArgv` is empty)
