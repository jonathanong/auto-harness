...
`pnpm check` runs oxlint, oxfmt, typecheck, vitest — both the unit project (**lines 98 / branches 97 / functions 100 / statements 98** on `modules/*/src/**/*.{ts,tsx}` and `services/*/src/**/*.{ts,tsx}`, excluding tests, type-only files, thin CLIs, and the short named deny-list in `vitest.config.ts`) and the integration project (`integration/`, no coverage gate of its own) — the Dynamo adapter verifier, knip, dependency-cruiser, lychee, and [`no-mistakes`](https://github.com/jonathanong/no-mistakes).
