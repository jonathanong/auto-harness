...
| `fleet:inventory`      | `PUT`/`DELETE` `/hosts/:id/inventory` and `/host-inventories` (attach repos/worktrees, labels, required environment, and provider-account attachments). Does **not** accept setup-script or executable-path edits.                                    |
