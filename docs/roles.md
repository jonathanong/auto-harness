...
| `sessions:archive`     | `POST /sessions/:id/archive`. Archive availability/download `GET` is an authenticated read.                                                                                                                                                                                                                                                                             |
...
| `accounts:write`       | User and service-account CRUD, including manual service-account credential rotation: create a new key, update consumers, then delete the old key ([auth.md](auth.md#service-accounts-api-keys)). Schedules and unreleased principal session drains stay on the old principal, so they must be recreated or released before `DELETE` succeeds. `GET` of those lists too. |
| `integrations:write`   | Slack, custom-webhook, and GitHub ingress configuration CRUD, including `GET`. Also `PUT /session-log-settings` (GET of those knobs is an authenticated read).                                                                                                                                                                                                          |
| `audit:read`           | `GET /audit-logs`.                                                                                                                                                                                                                                                                                                                                                      |
| `scheduler:run`        | `POST /scheduler/*` (assign, ack-deadlines, reclaim-stale). Internal.                                                                                                                                                                                                                                                                                                   |
| `agent:protocol`       | `POST /host/messages`, the host WebSocket, and host `PUT` of session log parts/archives. Requires `kind=service-account` **and** `boundHostId`.                                                                                                                                                                                                                         |
...
| Archive write       | `POST` requires `sessions:archive` **and** repo/host access.                                                                                                                                                                 |
| Archive read        | `GET` requires authentication and repo/host access, like session-log reads.                                                                                                                                                  |
...
| Recover a stuck Provider Account lease                           | `operator`   | usually unscoped          |
