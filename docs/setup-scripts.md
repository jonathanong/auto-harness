...
| Behavior          | Contract                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| ----------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| When setup runs   | Fresh sessions only. Native resumes skip every setup script. When a fresh session's fingerprint of the checked-out ref, the setup scripts that would run, any operator-declared extra checkout paths, any operator-declared host-absolute files, and the filtered child environment (`createChildEnv()` baseline plus allowlisted keys) matches the last successful setup for that worktree, the daemon skips those scripts, restores the captured exports (minus `HARNESS_*`), and logs `Setup unchanged; skipping.` After the assigned command is authorized to spawn, that sidecar is dropped so a later fresh session cannot skip against leftover ignored outputs such as `node_modules`. |
...
| Environment       | Successful exports flow to the next setup script and form the assigned command's base environment, except reserved `HARNESS_*` values. A cache hit restores that captured environment from a mode-0600 host sidecar instead of re-running setup. Changing the filtered child environment after a daemon restart (rotated allowlisted tokens, `PATH`, or removed variables) is a cache miss. Captured environment values are not written to session logs.                                                                                                                                                                                                                                       |
...
- Do not expect Auto Harness to hash `package.json`, lockfiles, sourced host files, or other paths
  unless you listed those paths as setup cache inputs or setup cache host inputs. Each declared
  path must be a regular file of at most 16 MiB; missing, non-regular, oversized, or unreadable
  files are cache misses and run setup again. If setup rewrites a declared extra, that run is not
  stored, so the next fresh session re-runs setup. Changing an undeclared file does not itself
  invalidate cached setup; after the assigned command can spawn, the sidecar is already dropped.
  Relative checkout extras stay relative: no `..` and no absolute checkout paths.
...
- If setup is expensive, have you declared the extra checkout paths that should invalidate cached
  setup (for example a lockfile) and any host-owned files the script sources? The host never
  auto-detects manifests or sourced files. A later assigned command still drops the sidecar so
  leftover ignored outputs cannot be reused on the next fresh session.
