...
Route groups map 1:1 to handler modules under `packages/api/src/handlers/rest/`.
...
- `packages/api/src/db/client.ts` — DynamoDB Document Client
- `packages/api/src/services/scheduler.ts` — assignment logic
- `packages/api/src/services/session-service.ts` — status transitions, validation
- `packages/api/src/services/notification.ts` — Slack thread updates
- `packages/shared` — types, constants, Zod (or equivalent) schemas
