...
Assignment consumes a K-way merge of per-shard priority/FIFO heads rather than draining shard 0…N. Writers persist `queueOrder` before readers query `statusShard-queueOrder`; active queued rows are backfilled, and a missing/backfilling index falls back to `statusShard-createdAt` plus in-memory `compareSessionsForQueue`. Conditional worktree/main-checkout writes remain the claim.
