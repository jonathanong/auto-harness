...
while that exact legacy generation still wins. An empty transcript on that path re-checks the live
claim before the object PUT. Authorized session reads use a durable metadata point
