...
| xterm.js in UI — current                  | Read-only ANSI/progress rendering with search, sizing, fullscreen, and download over the assigned CLI's PTY-backed output                                                                                                                                           |
| Session `source`                          | Audit and filter by api / ui / webhook / schedule                                                                                                                                                                                                                   |
| Agent auto-update drains                  | Signed-manifest orchestration drains, waits, verifies, stages, activates, and requests restart through injected boundaries; production fetch/install/supervisor adapters remain intentionally unwired, so operators still execute the manual path                   |
