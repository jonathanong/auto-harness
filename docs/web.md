...
| **Infrastructure retry**      | A queued session that consumed its one bounded infrastructure retry shows a warning with the cause (checkout fetch or host lost before launch). The session detail includes the retry count and last retry reason; a second eligible failure is terminal.                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| **Details** | Repository, ref, host, worktree ("Main checkout" for scheduled sessions), parent/root lineage links, target/fallback chain and resolved route, priority, concurrency id, created/started/completed timestamps, timeout (humanized, with a remaining-time bar while running), session usage, and a bounded live/paginated direct-child list                                             |
...
replace the data-loss state with an empty transcript. An in-flight processing retry claim is not
expired out from under its upload; empty completes against an already-expired row stay rejected.
...
| Button           | Behavior                                                                                                                                                                                                                                                                                                                                                                                                     |
| ---------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Resume**       | `POST /sessions/:id/resume` — new session pinned to the same host; any eligible worktree checks out the source ref when present (otherwise default branch), skips setup, and uses frozen `resumeArgvTemplate` argv only when both `cliResumeRef` and the template exist. Other resumes use live routing; a deleted Command falls through to a live fallback instead. Optional prompt, timeout, and priority. |
