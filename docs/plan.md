...
| D4  | `command` is a **named profile** resolved against agent config, not a free string passed over the API. _(Superseded: a session/schedule now targets `providerAccountId` or `commandId` against the global Provider/Provider Account/Command catalogs — see `docs/terminology.md` — resolved control-plane-side into a fixed `resolvedArgv` the agent just spawns; the substance below is unchanged.)_ | A free string is a straight line from "operator role" to arbitrary command execution on the VPS, and string→argv construction is its own injection surface. Naming profiles server-side (agent-side) closes both at once and shortens the API.                                                                                                                                                                                                               | Free-form `command` strings. Shell strings. `shell: true` anywhere in the executor.                                                                                                |
...
- An unknown `commandProfile` is rejected before spawn, with no shell fallback (D4). **Superseded**
  ...
  targets exactly one of `providerAccountId` or `commandId`, and an unknown one is rejected at
