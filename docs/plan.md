...
sessions and settles paused only after leases release. Missing admission state is invalid
(closed), and cron occurrences skipped while closed are never caught up. The supported automation
...
| D7  | The credential the agent uses for GitHub writes is a **fine-grained token scoped to one repository**: contents + pull-requests + issues write, no Actions/workflow write, no secrets read; or a GitHub App installation token minted per session, scoped to that session repository with the same permission set, and a run clamped to its lifetime.                                                                | This is the primary mitigation for D1 and D4 relaxing the trust boundary — a prompt is attacker-influenced (it can originate from an issue comment or embed captured CI output), so bound what a compromised session can reach.                                                                                                                                                                                                                              | Broad PATs. App installations broader than the repositories a host serves. App private keys held by the control plane. Reusing a human operator's token.                           |
...
| D9  | No Docker (or other sandbox) wraps the agent process itself. The host is trusted; the AI CLI runs directly on it, subject to the CLI's _own_ sandboxing (approval policy, writable-roots, hooks) which is configured per-repo, not by Auto Harness.                                                                                                                                                                 | Stated existing principle ([architecture/decisions.md](architecture/decisions.md), [security.md](security.md)) — keep it explicit here so it isn't "improved" into per-session containers as part of this work.                                                                                                                                                                                                                                              | Per-session containers, gVisor, rootless podman, or any other isolation layer wrapping the agent or its worktrees.                                                                 |
| D10 | Each logical session receives at most one automatic infrastructure retry. Only a checkout-stage fetch failure or an acknowledged host lost before the command-start acknowledgement is eligible; a retry keeps the same session, concurrency lock, inputs, and absolute queue deadline while receiving a fresh attempt fence. A second eligible infrastructure failure is terminal.                                 | These failures are safe to replay before the primary CLI could act, and the hard cap bounds duplicate side effects and queue work. The pre-launch checkpoint makes host loss auditable; ambiguous or post-launch loss remains terminal.                                                                                                                                                                                                                      | Retrying ordinary CLI/setup failures, usage limits, timeouts, cancellations, post-launch host loss, extending queue TTL, or creating a third attempt.                              |
| D11 | A terminal session may carry a bounded structured result alongside usage: agent/harness summary plus best-effort branch, changed files, and associated PR URL. The host observes this after the terminal hook; the accepted attempt stores it atomically, while session lists omit it. The dispatch action remains fire-and-forget and offers a separate one-shot `get-result`, never a polling loop.               | Decomposed and asynchronous callers need a machine-readable outcome without parsing transcripts. Bounded collection preserves storage and request limits.                                                                                                                                                                                                                                                                                                    | Requiring a special output format from every CLI. Inferring arbitrary transcript prose. Holding the dispatch job open or adding action-owned polling.                              |
...
erDiagram
    ...
    Session {
        ...
        string errorCode "nullable, e.g. usage_limit, queue_expired, checkout_fetch_failed, host_lost"
        string errorMessage "nullable"
        number infrastructureRetryCount "nullable; automatic infrastructure retries consumed, 0 or 1"
        string lastInfrastructureErrorCode "nullable; checkout_fetch_failed or host_lost"
    ...
    Schedule {
        ...
        string principalId "required authenticated owner"
...
**Active-host index rollout:** `activeHostId-activeHostOrder` is sparse and is populated by the
new assignment writers only. It has no scan fallback: cut over to readers only in a fresh
environment after confirmed work has drained (or after an explicit, resumable administrative
backfill has completed). Do not deploy its readers into an old environment that still has active
assignments without these attributes.
...
- **Bounded infrastructure retry** (D10): retry one exact checkout-fetch failure or one host loss
  proven to be before primary CLI launch. Preserve the session's concurrency lock, inputs, and
  absolute queue deadline; a second eligible failure is terminal.
