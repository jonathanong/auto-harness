...
in the same transaction as the lock Put. The storage-less local path checks the in-memory GitHub
comment lock before rejecting a stale fence, matching that 202 dedupe. Marker-only conflicts with no active lock owner still fail
...
binding catalog references after secret encryption, then check those maps again synchronously
immediately before commit so a concurrent repository, provider, or command deletion cannot land a
dangling binding. Durable writes skip that second catalog refresh; deletion markers plus the config
Put remain the fence. Configuration updates carry the operator's
