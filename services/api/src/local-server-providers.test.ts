...
describe("createLocalApp providers/provider-accounts/commands REST", () => {
  it("providers, provider accounts, and commands via handlers", async () => {
    ...
    expect(cmd.json).toMatchObject({ name: "echo hi", appendPrompt: true, providerId: null });
    ...
    expect(
      (await invoke("PATCH", `/api/v1/commands/${cmdId}`, { appendPrompt: false })).json,
    ).toMatchObject({ appendPrompt: false });
