...
describe("DynamoDB Slack integration storage", () => {
  it("uses conditional create/update/delete for the singleton integration", async () => {
    if (!ctx.storage) {
    ...
    expect(
      await ctx.storage.putSlackIntegration({ ...record, version: 2, enabled: false }, 1),
    ).toBe(true);
    expect(await ctx.storage.deleteSlackIntegration(1)).toBe(false);
    expect(await ctx.storage.deleteSlackIntegration(2)).toBe(true);
    expect(await ctx.storage.getSlackIntegration()).toBeNull();
  });
});
