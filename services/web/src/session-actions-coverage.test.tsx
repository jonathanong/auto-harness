...
describe("SessionActions", () => {
  ...
  it("clones a terminal session and navigates to the new session", async () => {
    ...
    const fetchMock = vi
      .fn()
      .mockResolvedValue(response(true, JSON.stringify({ id: "clone/session" })));
    vi.stubGlobal("fetch", fetchMock);
    const view = mount(
      <SessionActions sessionId="old" status="failed" detailHrefBase="/runs" />,
      router,
    );
    ...
    expect(fetchMock).toHaveBeenCalledWith("/api/v1/sessions/old/clone", { method: "POST" });
    expect(router.push).toHaveBeenCalledWith("/runs/clone%2Fsession");
    expect(router.refresh).not.toHaveBeenCalled();
    view.unmount();
  });
