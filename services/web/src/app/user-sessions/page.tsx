...
function roleLabel(role: UserRole | null): string {
  return role && role in USER_ROLE_LABELS ? USER_ROLE_LABELS[role] : "—";
}
...
export default async function UserSessionsPage() {
  ...
  return (
    <div className="space-y-4" data-pw="page-user-sessions">
      ...
      {error ? (
        <ListApiError resource="user sessions" message={error} selector="user-sessions" />
      ) : items.length === 0 ? (
          ...
          <TableBody>
            {items.map((item) => (
              <TableRow key={item.id} data-pw={`user-session-row-${item.id}`}>
                ...
                <TableCell data-pw={`user-session-role-${item.id}`}>
                  {roleLabel(item.role)}
                </TableCell>
                <TableCell
