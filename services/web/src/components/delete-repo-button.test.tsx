...
describe("DeleteRepoButton", () => {
  ...
  it("confirms a successful deletion with pending state and navigates", async () => {
    ...
    const view = mountForm(<DeleteRepoButton repositoryId="repo/1" attachedHostCount={0} />);
    open(view);
    press(confirm());
  ...
  it("shows a response body when deletion fails and supports cancel", async () => {
    vi.stubGlobal(
      "fetch",
      vi.fn().mockResolvedValue(new Response("repository has attachments", { status: 409 })),
    );
    const view = mountForm(<DeleteRepoButton repositoryId="repo/1" attachedHostCount={1} />);
    open(view);
    press(confirm());
