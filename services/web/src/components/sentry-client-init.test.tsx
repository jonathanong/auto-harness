...
describe("SentryClientInit", () => {
  it("inits the browser SDK once mounted", async () => {
    const container = document.createElement("div");
    document.body.append(container);
    const root = createRoot(container);
    await act(async () => {
      root.render(<SentryClientInit dsn="https://abc123@o1.ingest.sentry.io/450" plane="web" />);
    });
    await act(async () => {
      await Promise.resolve();
    });
    expect(initBrowserSentry).toHaveBeenCalledWith("https://abc123@o1.ingest.sentry.io/450", "web");
    await act(async () => {
      root.unmount();
    });
  });
