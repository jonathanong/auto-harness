...
function filledValue(key: string, fallback: string, env: NodeJS.ProcessEnv): string {
  ...
  if (key === "PATH") {
    return pathFromEnv(env) || fallback;
  }
  const value = env[key];
 ...
 */
export function renderEnvFile(example: string, env: NodeJS.ProcessEnv): string {
  ...
  for (const line of example.split(/\r?\n/)) {
    ...
    const key = line.slice(0, eq);
    seen.add(key);
    const value = filledValue(key, line.slice(eq + 1), env);
