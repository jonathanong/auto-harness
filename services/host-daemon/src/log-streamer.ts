...
// Half of API Gateway's 128 KB WebSocket message limit, leaving room for the JSON envelope
// (sessionId, stream, timestamp, seq) that wraps the raw log content on the wire. Deployed
// against AWS, a chunk over this size is rejected by the socket; the local `ws` transport has
// no such limit, so this was previously invisible outside a real deploy.
