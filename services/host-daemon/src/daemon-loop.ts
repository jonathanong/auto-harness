...
export type DaemonLoopOptions = {
  ...
  /** Bound on local-age fallback when a handoff has no absolute expiry. */
...
export class DaemonLoop {
  ...
  async applyInventory(
    next: DaemonConfig,
    options: { publishRegistration?: boolean } = {},
  ): Promise<void> {
  ...
  private async applyInventoryCandidate(
    ...
    if (
      next.inventoryVersion !== undefined &&
      this.config.inventoryVersion !== undefined &&
      next.inventoryVersion < this.config.inventoryVersion
    ) {
      return;
    }
  ...
  private sendCommandStart(key: string, pending: PendingCommandStart): void {
    if (pending.sending || pending.signal.aborted || this.pendingCommandStarts.get(key) !== pending)
      return;
  ...
  private async handleStatusAcknowledged(
    ...
    const acknowledge = (
      ...
      const syntheticV7Handoff =
        msg.terminalHookHandoffId !== undefined && msg.retryAccepted !== true;
    ...
    if (!refresh) {
      refresh = (async () => {
        ...
        try {
          ...
          const next = await this.waitForSignal(
            loadInventory(controller.signal),
            controller.signal,
          );
          await this.applyInventory(next, {
  ...
  private async runAssign(
    ...
    const statusMessage: Extract<HostToServerMessage, { type: "session:status" }> = {
      ...
      ...(result.result !== undefined && !settleDeferredTerminalHook
